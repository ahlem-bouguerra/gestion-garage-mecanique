...
const MecaniciensManager = () => {
  ...
  const mecaniciensApi = {
    getAll: async (): Promise<Mecanicien[]> => {
      ...
      if (!response.ok) throw new Error('Erreur lors de la récupération des mécaniciens');
      return await response.json();
    },

    getById: async (id: string): Promise<Mecanicien> => {
      const response = await fetch(`${API_BASE_URL}/${id}`);
      if (!response.ok) throw new Error('Mécanicien non trouvé');
      return await response.json();
    },

    create: async (data: FormData): Promise<Mecanicien> => {
      const response = await fetch(`${API_BASE_URL}/`, {
        method: 'POST',
    ...
    update: async (id: string, data: FormData): Promise<Mecanicien> => {
      const response = await fetch(`${API_BASE_URL}/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
  ...
  const saveMecanicien = async (): Promise<void> => {
    try {
      ...
      if (isEditMode && selectedMecanicien) {
        await mecaniciensApi.update(selectedMecanicien._id, formData);
        showSuccess('Mécanicien modifié avec succès !');
      } else {
        await mecaniciensApi.create(formData);
        showSuccess('Mécanicien créé avec succès !');
      }

      resetForm();
      await loadMecaniciens(); // Reload data from API
      setActiveView('grid');
  ...
  const startEdit = (mecanicien: Mecanicien): void => {
    ...
    setFormData({
      ...
      salaire: mecanicien.salaire || '',
      competences: mecanicien.competences || [],
      experience: mecanicien.experience || '',
      permisConduire: mecanicien.permisConduire || 'B',
      visiteMedicale: mecanicien.visiteMedicale ? mecanicien.visiteMedicale.split('T')[0] : '',
      equipementsAttribues: mecanicien.equipementsAttribues || []
    });
  ...
  const addCompetence = (): void => {
    setFormData({
      ...formData,
      competences: [...formData.competences, { domaine: 'Moteur', niveau: 'Débutant' }]
    });
  };
  ...
  const updateCompetence = (index: number, field: keyof Competence, value: string): void => {
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${
                        posteBadgeColors[mecanicien.poste] || 'bg-gray-100 text-gray-800 border-gray-200'
                      }`}>
                        {mecanicien.poste}
                      </span>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${
                        statusColors[mecanicien.statut] || 'bg-gray-100 text-gray-800 border-gray-200'
                      }`}>
                        {mecanicien.statut}
                      </span>
                    </div>
                    ...
                    {mecanicien.competences && mecanicien.competences.length > 0 && (
                      <div className="pt-3 border-t border-gray-100">
                        <p className="text-xs text-gray-500 mb-2">Compétences principales</p>
                        <div className="flex flex-wrap gap-1">
                          {mecanicien.competences.slice(0, 2).map((comp, index) => (
                            <span key={index} className="bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs">
                              {comp.domaine}
                            </span>
                          ))}
                          {mecanicien.competences.length > 2 && (
                            <span className="text-xs text-gray-500">+{mecanicien.competences.length - 2}</span>
                          )}
                        </div>
                      </div>
                    )}
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      value={formData.visiteMedicale}
                      onChange={(e) => setFormData({...formData, visiteMedicale: e.target.value})}
                      className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                    />
                  </div>
                  <div className="md:col-span-2">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 py-2 rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 flex items-center space-x-2 shadow-sm"
                  >
                ...
                <div className="space-y-4">
                  {formData.competences.map((competence, index) => (
                    <div key={index} className="flex items-center space-x-4 bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                      ...
                      <div className="w-32">
                        ...
                        <select
                          ...
                          onChange={(e) => updateCompetence(index, 'niveau', e.target.value)}
                          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        >
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-screen overflow-y-auto">
              ...
              <div className="p-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${
                          posteBadgeColors[selectedMecanicien.poste] || 'bg-gray-100 text-gray-800 border-gray-200'
                        }`}>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${
                          statusColors[selectedMecanicien.statut] || 'bg-gray-100 text-gray-800 border-gray-200'
                        }`}>
                      ...
                      <div className="flex justify-between">
                        <span className="font-medium text-gray-600">Dernière visite médicale:</span>
                        <span className="text-gray-900">{
                          selectedMecanicien.visiteMedicale ? 
                          new Date(selectedMecanicien.visiteMedicale).toLocaleDateString('fr-FR') : 'Non renseignée'
                        }</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Compétences */}
                  {selectedMecanicien.competences && selectedMecanicien.competences.length > 0 && (
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-100">
                      ...
                      <div className="space-y-3">
                        {selectedMecanicien.competences.map((comp, index) => (
                          <div key={index} className="flex justify-between items-center bg-white p-3 rounded-lg shadow-sm border border-purple-100">
                            <span className="font-medium text-gray-800">{comp.domaine}</span>
                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                              comp.niveau === 'Expert' ? 'bg-green-100 text-green-800 border border-green-200' :
                              comp.niveau === 'Confirmé' ? 'bg-blue-100 text-blue-800 border border-blue-200' :
